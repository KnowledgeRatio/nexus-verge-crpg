...
Top-down view, hand-painted cartographic atlas style — like a field survey illustrated in oil paint. Visible brushwork, painted texture throughout. NOT pixel art. NOT isometric. NOT sprites. NOT pixel graphics. Organic and earthy, each terrain a fragment of another world that took root: flora from two climates, geology that does not match its neighbours. Biome accent colours: marshes use sickly yellow-green, forests use rich deep teal shadow, mountains use blue-grey with frost accents, plains use warm pale straw. No characters or creatures. Seamless edges. Absolutely no grid lines, no graph paper, no graph texture, no ruled lines, no grid overlay, no squared paper look. At biome boundaries, render a visible convergence seam — an abrupt material shift where two terrain types meet, not a gradient blend.
