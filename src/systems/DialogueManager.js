...
export default class DialogueManager {
    ...
    getGreeting(npc, tone) {
        const line = this._selectGreetingLine(npc, tone);
        return this._fillTemplate(line, {
            name: npc.name,
            settlement: npc.settlementName || '',
            title: npc.role,
            tavernName: npc.shopName || '',
            shopName: npc.shopName || ''
        });
    }

    /**
     * Select a raw (unfilled) greeting line for an NPC, before template substitution
     * @param {Object} npc - NPC object
     * @param {string} tone - Relation tone
     * @returns {string} Raw greeting line, possibly containing {placeholder}s
     */
    _selectGreetingLine(npc, tone) {
        if (!this.dialogueData?.greetings) {
            return npc.dialogue?.greeting || 'Hello.';
        }

        // Culture should be audible before the name is stated (docs/world/PEOPLES.md) —
        // try a culture-specific greeting before falling through to the generic pools.
        const cultureLine = this._pickCultureGreeting(npc, tone);
        if (cultureLine) {
            return cultureLine;
        }
    ...
    /**
     * Try a culture-specific greeting for this NPC's role/personality/tone, falling
     * back to that culture's neutral tone. Returns null if no culture data exists for
     * this exact combination, so callers fall through to the generic pools.
     * @param {Object} npc - NPC object (needs npc.culture)
     * @param {string} tone - Relation tone
     * @returns {string|null} Greeting text, or null if no culture-specific line exists
     */
    _pickCultureGreeting(npc, tone) {
        const cultureGreetings = this.dialogueData?.greetingsByCulture?.[npc.culture]?.[npc.role]?.[npc.personality];
        if (!cultureGreetings) {
            return null;
        }

        const toneLines = cultureGreetings[tone];
        if (toneLines && toneLines.length > 0) {
            return toneLines[Math.floor(Math.random() * toneLines.length)];
        }

        const neutralLines = cultureGreetings['neutral'];
        if (neutralLines && neutralLines.length > 0) {
            return neutralLines[Math.floor(Math.random() * neutralLines.length)];
        }
    ...
    getFlavorLines(npc, tone, count = 2) {
        ...
        // 1b. Culture-specific lines (weight: 3 — same priority as tone)
        const culturePool = this.dialogueData?.flavorDialogueByCulture?.[npc.culture];
        if (culturePool) {
            for (const line of culturePool) {
                candidates.push({ line, weight: 3 });
            }
        }
