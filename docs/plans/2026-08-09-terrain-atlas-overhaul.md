...
- **Open question flagged for `game-designer`, not resolved here:** `sanctuary`'s standalone `terrains.json` entry (art now written above) is also what `MapRenderer.js` currently falls back to when a POI resolves to `sanctuary` and gets revealed — meaning a revealed POI (e.g. a `monastery` that turns out to be a sanctuary) may visibly switch from its own POI art to generic `sanctuary` art at the reveal moment, rather than keeping its POI-type art forever. Whether that's intended or a bug is a mechanic question, independent of this session's art work — this session's `sanctuary` `imageDescription` was written as a self-contained landmark (small roadside waystation shelter) without trying to pre-resolve that question either way.
- **Blocked:** `npm run gen:terrain` requires `AZURE_FOUNDRY_ENDPOINT`/`AZURE_FOUNDRY_API_KEY`, not set in this environment. No images generated yet — someone with credentials needs to run it, for tier-1 and all of this session's tier 2/3/4/POI work alike.
- `npx vitest run tests/data/terrainVisuals.test.js` still shows the same 1 expected failure as tier-1 (existsSync fails on the first `continuousAtlas` entry missing generated art) — unchanged in count since it's a single `it()` block that stops at the first failing assertion, not a per-terrain failure count. Not a new regression from this session's additions.
