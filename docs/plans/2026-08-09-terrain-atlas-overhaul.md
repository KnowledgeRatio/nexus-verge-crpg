...
- ~~**Still open:** the dungeon-tile follow-up pass~~ — **done 2026-09-08.** Audit confirmed `DungeonUI.js` draws every dungeon tile with `fillRect` + `fillText` only; its sole `new Image()` is the player avatar. No code path reads a `dungeon*` `tileImage`. All 16 `dungeon*`/`beach` entries had their `tileImage` set to `null` and the 16 files were deleted. This also fixed a live waste: `MapRenderer.loadTileImages()` preloads *every* terrain with a `tileImage`, so the overworld renderer had been loading 15 dungeon images it can never draw — preload is now 32 images instead of 48.
- **Superseded `-mai-2.png` art deleted (2026-09-08).** All 27 remaining legacy-style terrain images were unreferenced (no `tileImage` pointed at any of them) and each had a shipped `-cartographic-*-v1.png` replacement. Removed, along with their stale `tools/image-gen/manifest.json` ledger entries. `data/graphics/` went from 197 MB to 135 MB. All files were git-tracked, so this is recoverable from history if the old visual language is ever wanted back.
- **Retired POI type ids removed from the legacy path (2026-09-08).** `WorldGenerator.js`'s dead non-finite-world fallback still built POIs from a hardcoded `['shrine', 'ruins', 'cave', 'camp', 'landmark']` — `shrine` and `landmark` are retired and have no `terrains.json` entry, so they would have rendered as `?` had that path ever run. It now derives the list from `this.poiData.types` (ADR-010, data drives code), which cannot drift from `pois.json` again, and skips POI placement entirely if POI data failed to load.

**Deliberately not done — `beach` is retired but not removable cheaply.** Worldgen never assigns it (`rulesEngine.js`: "beach retired - not orientation-safe"), and its art is now gone, but the id is still referenced in ~14 `skillChallenges.json` `terrainModifiers` maps, a `dungeons.json` theme, `rulesEngine.js`'s `beaches` config block, a colour entry in `main.js`, a `DialogueManager.js` mapping, and warn-once defensive code in `MapRenderer.js`. Removing the terrain entry itself is a multi-file change with real regression surface and is a separate decision, not cleanup.
...
**Second pass (2026-09-08) — `temple`, `monastery`, `watchtower` regenerated.** Root cause of the persistent contour/halo artifact was self-inflicted: the ban added to `style-guide.md` in the first pass *named* the artifact ("no concentric contour rings, topographic contour lines, ripple bands, halo, vignette"), and since those clauses ride in the positive prompt, they summoned it. That paragraph is now a positive assertion (one even field, same tone and density at centre, edges and corners, marks pointing in no shared direction), and the same negation-to-assertion swap was applied per entry: "never at an angle or isometric" became "every shape lies flat in the picture plane, as on a surveyor's plan"; the relief-word pile became "one flat level plane throughout"; "never a dark opening, shaft, hole, or void" became "a pale solid stone floor, filled in light". Subject sizes were also raised — the earlier "roughly two fifths / one third of the tile width" phrasing rendered far smaller than stated, so both are now framed as "large in frame and dominates the tile". Result: subject scale and flatness fixed on all three; a soft brushy swirl remains in temple's and watchtower's ground at full size, which blurs into ordinary painted texture at play scale, and monastery carries a faint edge rim.

**Azure content-safety wall on `monastery` — worth knowing before regenerating it.** Three consecutive attempts died on `Azure API 400 content_safety_violation … BingBlockList`, across three different prompt texts, while `temple` and `watchtower` (longer prompts) passed. `BingBlockList` filters the generated *response*, so this was a visual match, not a banned word: a walled compound of uniform rooms around a courtyard seen from directly overhead resembles a blocked real-world subject. Reframing the same composition as a **hill farmstead of slate-roofed halls around a yard** — identical hollow-rectangle silhouette, identical terrace beds — cleared it first try. If monastery is ever regenerated, keep the farmstead framing; do not restore "walled discipline house" or "walled stone compound".

**Canon rebrand shipped alongside (2026-09-08).** `docs/world/GODS.md`'s "No pantheon. No divine casters. No church." was being contradicted by live content. Fixed, strings only, no mechanical change: the `holy_ritual` skill challenge is now `votive_record` / "Read the Votive Record" (same skill, DC, loot table and triggers; id updated in `dungeonTypes.json` ×2 and `Player.js`); `pois.json` naming pools lost "the Everflame", "the Silent Order" (which also collided with the Vethri exonym), and the Blessed/Sacred/Faithful/Devoted/Grace/Chapel register; `ruins` gave up its shrine vocabulary to stop duplicating `temple`; `villa` lost "Noble"/"Old Wealth" (no hereditary nobility in canon); `watchtower`'s exclusively imperial-Vaethoran pool was diluted with Lookout/Beacon/Highpost so it doesn't read as a standing imperial garrison on Verathi or Vethri ground; `dungeons.json`'s cult theme lost the orthodoxy-relative words (Profane/Heretical/Unholy) and Christian-specific architecture (Cathedral/Tabernacle/Chapel) while keeping the cult itself, which *is* canon; `quests.json` lost its "temple keeper" clergy.

**Display-name rename — the gotcha.** `temple` now displays as **Shrine** and `monastery` as **Cloister**, in both `terrains.json` and `pois.json`. **The type ids are unchanged** (`temple`, `monastery`) and remain what `WorldGenerator`, `biomeEligibility`, `skillChallenges` triggers and `quests.json` `locationTypes` match on. Do not "fix" the id to match the label.

**Left alone deliberately:** SRD 5e mechanical content keeps its vocabulary — Sacred Flame, Holy Symbol, Priest's Pack, Holy Water, the flameskull stat block — since that is D&D compliance, not Verge canon. The SRD **Acolyte background** text in `backgrounds.json` does assert temples, a pantheon and a faith, which is the one unresolved conflict of this class; it is SRD-derived and player-facing at character creation, so it needs a deliberate call rather than a quiet rewrite.
