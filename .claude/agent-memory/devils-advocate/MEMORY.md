...
### 17. Attribute-Remap Dead-Shim Trap (2026-08-01)
- `docs/plans/2026-07-30-attribute-system-remap.md` decision #2 (Composure = averaged WIS+CHA save) built `monsterAttributeConversion.js`'s `convertMonsterSavingThrows()`, attached to enemy objects as `character.savingThrows` (`EncounterBuilder.js:198`) — but the live save-roll path (`EffectDispatcher.rollDefenderSave()` → `getAttributeModifierFor('composureSave')` → `getRawAttributeModifier`) reads `character.abilities.composure` directly, never `character.savingThrows`. The averaging shim's output is dead — computed, tested, never consumed.
- Compounding: the separately-locked "Legacy split-attribute conversion" bijection (WIS→Composure, CHA→Presence, `rulesEngine.js` `legacyToNew`) means once M2 populates `abilities.composure` for real, it will be WIS-only — CHA never enters that number anywhere. This reproduces the exact "keep-higher discards WIS" failure mode the plan explicitly rejected, just inverted (discards CHA) and accidental, not deliberate. The code already half-flags this at `rulesEngine.js` `legacySaveAbilityToContext` comment ("CHA routing to composureSave... drops the separate CHA value... later backend-dev formula job") but it isn't tracked as an open task in the plan.
- Also: the averaging branch of `averageComposureSaveBonus` has zero real data to exercise even if wired correctly — the only monsters with both WIS-save and CHA-save (3 dragons) were separately flagged non-canon and dropped from scope in the same conversation. Zombie/mage (the real saves subset) each have only one of the two values.
- General pattern: when a plan locks a data-transform decision (e.g. "average these two values") and a separate wiring/architecture decision (e.g. "the resolver reads ability scores, not save overrides") independently, always check whether the transform's output is actually on the path the wiring reads — two correct decisions made in isolation can leave a computed value orphaned. Grep for actual consumers of a shim's output field before trusting the shim is live.
- Same review found `RULES.flee.modifier: ['dex','wis']` (`rulesEngine.js:1160`) is dead config too — `CombatManager.js`'s legacy flee branch hardcodes `dexMod`/`wisMod` directly instead of reading it. Matches pattern #8 (Dead Config Anti-Pattern) recurring in a third system.
- By contrast, decision #4 (flee formula converging to `floor((Prowess+Insight)/2)`, dropping the old `max(DEX,WIS)` universal-access guarantee) was found genuinely low-risk on inspection: no live ability/kit references flee+wis, and Dedication's `skillChoices` (`classes.json`) doesn't even include Creativity (Composure's one skill), so a Composure-stacking Dedication build has no skill support and would already be unplayable (worst attack/damage/AC-evasion/initiative) before flee is considered. Correctly wired end-to-end in both legacy and sixAttribute branches (`CombatManager.js:1774-1790`) — worth noting as the clean counterexample to the dead-shim above in the same review.
