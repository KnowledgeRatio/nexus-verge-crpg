...
- `SettlementManager._restoreSettlementFromPersistent(settlement)` — new helper, reads flags/localVisitCount/merchantLocked/sacked from `world.settlements[]` back onto live region feature before ConsequenceManager runs.
- `WorldGenerator.persistSettlementData()` — updated to include `flags`, `localVisitCount`, `merchantLocked`, `sacked` in the push block (update block uses spread so already covered).
...
- Condition floating texts (Sap, Slow, Topple, Push, Harried, Downed, Dodging) now pass `delay` as a property in the notify payload — the subscriber passes it as the 4th arg to `showFloatingCombatText`.
